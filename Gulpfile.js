...
gulp.task('compile-nunjucks', function() {
    ...
    gulp
        .src(path.join(DEV_DIR, 'app/pages/**/*.+(html|nunjucks)'))
...
gulp.task('default', ['serve-dist'], function() {
    gulp.watch(path.join(DESIGN_DIR, 'scss/**/*.scss'), [
        'stylesheets',
        'bundle-sw'
    ]);
    gulp.watch(path.join(DESIGN_DIR, 'js/**/*.js'), [
        'javascript',
        'bundle-sw'
    ]);
    gulp.watch(path.join(DEV_DIR, '**/*.+(html|nunjucks)'), [
        'compile-nunjucks',
        'bundle-sw'
    ]);
